    ...
    with open(file_name, 'w') as f:
        for idx in range(count):
            ...
            f.write('%s %f %f %f %f %f\n' % (
                label.replace(' ', '_'),
                obj.prob,
                int(w * obj.x), int(h * obj.x),
                int(w * obj.y), int(h * obj.y),
                int(w * obj.w), int(h * obj.w),
                int(w * obj.h), int(h * obj.h),
            ))

def write_predictionsHeader(frame, idx, category, probability, x, y, w, h):
