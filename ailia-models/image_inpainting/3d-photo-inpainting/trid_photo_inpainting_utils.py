...
def edge_forward_3P(net, mask, context, rgb, disp, edge, unit_length=128):
    ...
    edge_output = output[0]
    edge_output = edge_output[..., anchor_h:anchor_h + h, anchor_w:anchor_w + w]
    return edge_output


def depth_forward_3P(net, mask, context, depth, edge, unit_length=128):
    input = np.concatenate((depth, edge, context, mask), axis=1)
    n, c, h, w = input.shape
    residual_h = int(np.ceil(h / float(unit_length)) * unit_length - h)
    residual_w = int(np.ceil(w / float(unit_length)) * unit_length - w)
    anchor_h = residual_h // 2
...
def color_forward_3P(net, mask, context, rgb, edge, unit_length=128):
    input = np.concatenate((rgb, edge, context, mask), axis=1)
    n, c, h, w = input.shape
    residual_h = int(np.ceil(h / float(unit_length)) * unit_length - h)
    residual_w = int(np.ceil(w / float(unit_length)) * unit_length - w)
    anchor_h = residual_h // 2
...
def open_small_mask(mask, context, open_iteration, kernel):
    np_mask = mask.squeeze().astype(np.uint8)
    np_context = context.squeeze().astype(np.uint8)
    ...
    out_mask = np_mask.astype(mask.dtype)[None, None, ...]

    return out_mask


def filter_irrelevant_edge_new(
        self_edge, comp_edge, other_edges,
        other_edges_with_id, current_edge_id,
        context, depth, mesh, context_cc, spdb=False):
    ...
    dilate_bevel_self_edge = cv2.dilate((self_edge + comp_edge).astype(np.uint8),
...
def clean_far_edge_new(
        input_edge, end_depth_maps, mask, context,
        global_mesh, info_on_pix, self_edge, inpaint_id, config):
    mesh = netx.Graph()
    ...
    for hx, hy in zip(hxs, hys):
        ...
        eight_nes = [ne for ne in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1), \
                     ...
                     if 0 <= ne[0] < input_edge.shape[0] and 0 <= ne[1] < input_edge.shape[1] and 0 < input_edge[
        ...
        for ne in eight_nes:
            ...
            if end_depth_maps[ne[0], ne[1]] != 0:
                ...
                four_nes = [nne for nne in
                            [(ne[0] + 1, ne[1]), (ne[0] - 1, ne[1]), (ne[0], ne[1] + 1), (ne[0], ne[1] - 1)] \
                            if nne[0] < end_depth_maps.shape[0] and nne[0] >= 0 and nne[1] < end_depth_maps.shape[1] and
                            nne[1] >= 0]
                for nne in four_nes:
    ...
    while end_idx < len(end_pts):
        ...
        if len(end_pt) == 2:
            ...
            tmp_npath = [*netx.shortest_path(tmp_sub_mesh, (ravel_end[0][0], ravel_end[0][1]),
                                             (ravel_end[1][0], ravel_end[1][1]), weight='length')]
            fpath_map1, npath_map1, disp_diff1 = plan_path(
                mesh, info_on_pix, cc, ravel_end[0:1], global_mesh,
                input_edge, mask, valid_map, inpaint_id, npath_map=None,
                fpath_map=None, npath=tmp_npath)
            fpath_map2, npath_map2, disp_diff2 = plan_path(
                mesh, info_on_pix, cc, ravel_end[1:2], global_mesh,
                input_edge, mask, valid_map, inpaint_id, npath_map=None,
                fpath_map=None, npath=tmp_npath)
            ...
            sorted_end_pts = [xx[0] for xx in sorted(zip(ravel_end, self_end, edge_len, [disp_diff1, disp_diff2]),
            ...
            if global_mesh.nodes[ends[0]].get('far') is None:
                ...
                while True:
                    ...
                    ffnode = [fnode for fnode in fnodes if
                              (dmask[fnode[0], fnode[1]] > 0 and mask[fnode[0], fnode[1]] == 0 and \
...
def plan_path_e2e(
        mesh, cc, end_pts, global_mesh, input_edge,
        mask, valid_map, inpaint_id, npath_map=None,
        fpath_map=None):
    ...
    if global_mesh.nodes[ends_1].get('far') is None:
        ...
        while True:
            ...
            e_ffnode = [e_fnode for e_fnode in e_fnodes if
                        (dmask[e_fnode[0], e_fnode[1]] > 0 and mask[e_fnode[0], e_fnode[1]] == 0 and \
...
def plan_path(
        mesh, info_on_pix, cc, end_pt, global_mesh, input_edge,
        mask, valid_map, inpaint_id, npath_map=None,
        fpath_map=None, npath=None):
    my_npath_map = np.zeros_like(input_edge) - 1
    my_fpath_map = np.zeros_like(input_edge) - 1
    sub_mesh = mesh.subgraph(list(cc)).copy()
    pnodes = netx.periphery(sub_mesh)
...
def create_placeholder(
        context, mask, depth, fpath_map, npath_map, mesh, inpaint_id,
        edge_ccs, extend_edge_cc, all_edge_maps, self_edge_id):
    add_node_time = 0
    add_edge_time = 0
    add_far_near_time = 0
    ...
    for near_id in near_ids:
        ...
        for hx, hy in zip(hxs, hys):
            ...
            four_nes = [(x, y) for x, y in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1)] if \
                        x < mesh.graph['H'] and x >= 0 and y < mesh.graph['W'] and y >= 0 and npath_map[
                            x, y] == near_id]
            for xx in four_nes:
        ...
        for hx, hy in zip(hxs, hys):
            ...
            four_nes = [(x, y) for x, y in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1)] if \
                        x < mesh.graph['H'] and x >= 0 and y < mesh.graph['W'] and y >= 0 and fpath_map[
                            x, y] == near_id]
            for xx in four_nes:
                xx_n = copy.deepcopy(xx)
                if not mesh.has_node(xx_n):
...
def read_MiDaS_depth(
        depth, disp_rescale=10., h=None, w=None):
    disp = depth - depth.min()
...
def sparse_bilateral_filtering(
        depth, image, config, HR=False, mask=None, num_iter=None):
    """
    config:
    - filter_size
    """
...
def vis_depth_discontinuity(
        depth, config, vis_diff=False, label=False, mask=None):
    """
    config:
    -
    """
...
def bilateral_filter(
        depth, config, discontinuity_map=None, mask=None, window_size=False):
...
def rolling_window(a, window, strides):
    ...
    def acc_shape(i):
        if i + 1 >= len(a.shape):
            ...
            return functools.reduce(lambda x, y: x * y, a.shape[i + 1:])
...
def extrapolate(
    ...
    if "up" in direc.lower() and "-" not in direc.lower():
        ...
        all_anchor = [h_off + noext_H - config['context_thickness'], 2 * h_off + noext_H, 0,
                      w_off + config['context_thickness']]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [0, h_off + config['context_thickness'], w_off + noext_W - config['context_thickness'],
                      2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [h_off + noext_H - config['context_thickness'], 2 * h_off + noext_H,
                      w_off + noext_W - config['context_thickness'], 2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
    ...
    t_depth_zero_mean_depth = input_zero_mean_depth[None, None, ...].astype(np.float32)
    ...
    for hx, hy in zip(hxs, hys):
        ...
        eight_nes = [ne for ne in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1), \
                                   (hx + 1, hy + 1), (hx - 1, hy - 1), (hx - 1, hy + 1), (hx + 1, hy - 1)] \
                     if 0 <= ne[0] < output_raw_edge.shape[0] and 0 <= ne[1] < output_raw_edge.shape[1] and 0 <
                     output_raw_edge[ne[0], ne[1]]]
        for ne in eight_nes:
            mesh.add_edge(node, ne, length=np.hypot(ne[0] - hx, ne[1] - hy))
            if end_depth_maps[ne[0], ne[1]] != 0:
                mesh.nodes[ne[0], ne[1]]['cnt'] = True
    ...
    for end_pt, cc in zip(end_pts, ccs):
        ...
        if len(end_pt) == 1:
            ...
            pnodes = netx.periphery(sub_mesh)
            ends = [*end_pt]
            edge_id = global_mesh.nodes[(ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2], -ends[0][2])][
                'edge_id']
            pnodes = sorted(pnodes,
                            key=lambda x: np.hypot((x[0] - ends[0][0]), (x[1] - ends[0][1])),
                            reverse=True)[0]
            ...
            for np_node in npath:
                npath_map[np_node[0], np_node[1]] = edge_id
            fpath = []
            if global_mesh.nodes[(ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2], -ends[0][2])].get(
                    'far') is None:
                print("None far")
            else:
                fnodes = global_mesh.nodes[(ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2], -ends[0][2])].get(
                    'far')
                ...
                while True:
                    ...
                    ffnode = [fnode for fnode in fnodes if
                              (dmask[fnode[0], fnode[1]] > 0 and mask[fnode[0], fnode[1]] == 0)]
                    if len(ffnode) > 0:
                        fnode = ffnode[0]
                        break
                if len(ffnode) == 0:
                ...
                for step in range(0, len(npath) - 1):
                    ...
                    new_loc_nes = [
                        xx for xx in [
                            (new_loc[0] + 1, new_loc[1]), (new_loc[0] - 1, new_loc[1]),
                            (new_loc[0], new_loc[1] + 1), (new_loc[0], new_loc[1] - 1)] \
                        if xx[0] >= 0 and xx[0] < fpath_map.shape[0] \
                           and xx[1] >= 0 and xx[1] < fpath_map.shape[1]
                    ]
    ...
    depth_output = depth_forward_3P(
        depth_feat_model, t_mask, t_context, t_depth_zero_mean_depth, t_update_edge)
    ...
    rgb_output = color_forward_3P(rgb_feat_model, t_mask, t_context, t_rgb, t_update_edge)

    rgb_output = (
            (rgb_output.squeeze().transpose(1, 2, 0) * mask[..., None] + input_rgb) * 255).astype(
        np.uint8)
    image[all_anchor[0]:all_anchor[1], all_anchor[2]:all_anchor[3]][mask > 0] = rgb_output[
        mask > 0]  # np.array([255,0,0]) # rgb_output[mask > 0]
        ...
        for nex, ney in four_nes:
            if fpath_map[nex, ney] == n_id:
                na, nb = (nx + all_anchor[0], ny + all_anchor[2],
                          info_on_pix[(nx + all_anchor[0], ny + all_anchor[2])][0]['depth']), \
                         (nex + all_anchor[0], ney + all_anchor[2],
                          info_on_pix[(nex + all_anchor[0], ney + all_anchor[2])][0]['depth'])
    ...
    for nx, ny in zip(nxs, nys):
        self_node = (
            nx + all_anchor[0], ny + all_anchor[2], info_on_pix[(nx + all_anchor[0], ny + all_anchor[2])][0]['depth'])
...
def fill_dummy_bord(mesh, info_on_pix, image, depth, config):
    ...
    context[
    mesh.graph['hoffset']:mesh.graph['hoffset'] + mesh.graph['noext_H'],
    mesh.graph['woffset']:mesh.graph['woffset'] + mesh.graph['noext_W']
    ] = 1
    mask = 1 - context
...
def refresh_bord_depth(mesh, info_on_pix, image, depth):
    ...
    # (0, W - 1), (H - 1, 0), (H - 1, W - 1)]
    bord_nodes = []
    bord_nodes += [(mesh.graph['bord_up'], xx) for xx in
                   range(mesh.graph['bord_left'] + 1, mesh.graph['bord_right'] - 1)]
    bord_nodes += [(mesh.graph['bord_down'] - 1, xx) for xx in
                   range(mesh.graph['bord_left'] + 1, mesh.graph['bord_right'] - 1)]
    bord_nodes += [(xx, mesh.graph['bord_left']) for xx in
                   range(mesh.graph['bord_up'] + 1, mesh.graph['bord_down'] - 1)]
    bord_nodes += [(xx, mesh.graph['bord_right'] - 1) for xx in
                   range(mesh.graph['bord_up'] + 1, mesh.graph['bord_down'] - 1)]
...
def depth_inpainting(
        context_cc, extend_context_cc, erode_context_cc, mask_cc, mesh,
        config, union_size, depth_feat_model, edge_output,
        given_depth_dict=False, spdb=False):
    if given_depth_dict is False:
        depth_dict = get_depth_from_nodes(context_cc | extend_context_cc, erode_context_cc, mask_cc, mesh.graph['H'],
                                          mesh.graph['W'], mesh, config['log_depth'])
    ...
    tensor_depth_dict['output'] = \
        np.exp(depth_output + depth_dict['mean_depth']) * \
        tensor_depth_dict['mask'] + tensor_depth_dict['depth']
    patch_depth_dict['output'] = tensor_depth_dict['output'].squeeze()
    ...
    depth_output = smooth_cntsyn_gap(
        depth_dict['output'].copy() * depth_dict['mask'] + depth_dict['depth'] * depth_dict['context'],
    ...
    if spdb is True:
        ...
        ax2.imshow(depth_dict['output'] * depth_dict['mask'] + depth_dict['depth']);
        plt.show()
...
def update_info(mapping_dict, info_on_pix, *meshes):
    ...
    for mesh in meshes:
        rt_meshes.append(
            relabel_node(mesh, mesh.nodes, [*mapping_dict.keys()][0], [*mapping_dict.values()][0])
        )
...
def get_edge_from_nodes(
    ...
    rt_dict = {'rgb': rgb, 'disp': disp, 'depth': depth, 'real_depth': real_depth, 'self_edge': edge,
               'context': context,
               'mask': mask, 'fpath_map': fpath_map, 'npath_map': npath_map, 'comp_edge': comp_edge,
               'valid_area': context + mask,
...
def convert2tensor(input_dict):
    ...
    for key, value in input_dict.items():
        if 'rgb' in key or 'color' in key:
            rt_dict[key] = value.transpose(2, 0, 1)[None, ...]
        else:
            rt_dict[key] = value[None, None, ...]
...
def create_mesh(depth, image, int_mtx, config):
    ...
    for x, y, d in LDI.nodes:
        two_nes = [ne for ne in [(x + 1, y), (x, y + 1)] if
                   ne[0] < LDI.graph['bord_down'] and ne[1] < LDI.graph['bord_right']]
...
def tear_edges(mesh, threshold=0.00025, xy2depth=None):
    ...
    for edge in mesh.edges:
        if abs(mesh_nodes[edge[0]]['disp'] - mesh_nodes[edge[1]]['disp']) > threshold:
            ...
            mesh_nodes[far]['near'] = [] if mesh_nodes[far].get('near') is None else mesh_nodes[far]['near'].append(
                near)
...
def generate_init_node(mesh, min_node_in_cc):
    mesh_nodes = mesh.nodes
    ...
    for node in remove_nodes:
        ...
        for far_node in far_nodes:
            if mesh.has_node(far_node) and mesh_nodes[far_node].get('near') is not None and node in \
                    mesh_nodes[far_node]['near']:
                mesh_nodes[far_node]['near'].remove(node)
        near_nodes = [] if mesh_nodes[node].get('near') is None else mesh_nodes[node]['near']
        for near_node in near_nodes:
            if mesh.has_node(near_node) and mesh_nodes[near_node].get('far') is not None and node in \
                    mesh_nodes[near_node]['far']:
    ...
    for node in LDI.nodes:
        ...
        if len(neighbors) < 4:
            ...
            inval_diag_candi = set(
                [inval_diagonal for ne_node in discont_nes for inval_diagonal in LDI.neighbors(ne_node) if \
            ...
            for ne_node in diag_candi_anc:
                ...
                for diag_candi in LDI.neighbors(ne_node):
                    if [diag_candi[0], diag_candi[1]] in diagonal_xys and LDI.degree(diag_candi) < 4:
                        ...
                        if key_exist(LDI.nodes[diag_candi], 'must_connect') and node in LDI.nodes[diag_candi][
                            'must_connect'] and \
                                key_exist(LDI.nodes[node], 'must_connect') and diag_candi in LDI.nodes[node][
                            'must_connect']:
                            add_new_node(discont_graph, diag_candi)
                            add_new_edge(discont_graph, diag_candi, node)
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for fne in four_nes:
            if mesh.has_node((fne[0], fne[1], end_maps[fne[0], fne[1]])):
                ...
                mesh_nodes[node_b]['must_connect'] = set() if mesh_nodes[node_b].get('must_connect') is None else \
                ...
                mesh_nodes[node_a]['must_connect'] = set() if mesh_nodes[node_a].get('must_connect') is None else \
...
def remove_redundant_edge(
        mesh, edge_mesh, edge_ccs, info_on_pix, config,
        redundant_number=1000, invalid=False, spdb=False):
    point_to_amount = {}
    point_to_id = {}
    ...
    point_to_adjoint = {}
    for nx, ny in zip(nxs, nys):
        adjoint_edges = set([end_maps[x, y] for x, y in [(nx + 1, ny), (nx - 1, ny), (nx, ny + 1), (nx, ny - 1)] if
                             end_maps[x, y] != -1])
        point_to_adjoint[end_maps[nx, ny]] = (
                point_to_adjoint[end_maps[nx, ny]] | adjoint_edges) if point_to_adjoint.get(
            ...
            if len([*edge_mesh.neighbors(valid_edge_node)]) == 3:
                ...
                if invalid is False:
                    eight_nes = [(x, y) for x, y in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1),
                                                     ...
                                                     (hx + 1, hy - 1)] \
                                 if info_on_pix.get((x, y)) is not None and edge_canvas[x, y] != -1 and edge_canvas[
                                     x, y] != valid_edge_id]
                    if len(eight_nes) == 0:
                        end_number += 1
                if invalid is True:
                    ...
                    db_eight_nes = [(x, y) for x in range(hx - 2, hx + 3) for y in range(hy - 2, hy + 3) \
                                    if info_on_pix.get((x, y)) is not None and edge_canvas[x, y] != -1 and edge_canvas[
                                        x, y] != valid_edge_id and (x, y) != (hx, hy)]
                ...
                for ne in four_nes:
                    try:
                        if invalid is True or (
                                point_to_amount.get(ne) is None or point_to_amount[ne] < redundant_number) or \
...
def remove_dangling(mesh, edge_ccs, edge_mesh, info_on_pix, image, depth, config):
    ...
    for node in conn_1_nodes:
        hx, hy = node[0], node[1]
        eight_nes = set([(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
                         [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1),
                          (hx + 1, hy + 1), (hx - 1, hy - 1), (hx - 1, hy + 1), (hx + 1, hy - 1)] \
                         if info_on_pix.get((x, y)) is not None])
        ...
        ccs = netx.connected_components(sub_mesh)
        largest_cc = \
            sorted(ccs, key=lambda x: (len(x), -np.sum([abs(xx[0] - node[0]) + abs(xx[1] - node[1]) for xx in x])))[-1]
    ...
    for cc in ccs:
        ...
        for node in candidate_nodes:
            ...
            eight_nes = set([(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
                             [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1),
                              (hx + 1, hy + 1), (hx - 1, hy - 1), (hx - 1, hy + 1), (hx + 1, hy - 1)] \
                             if
                             info_on_pix.get((x, y)) is not None and (x, y, info_on_pix[(x, y)][0]['depth']) not in cc])
        ...
        if len(cc) == 1:
            ...
            hx, hy = node[0], node[1]
            nine_nes = set([(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
...
def context_and_holes(
        mesh, edge_ccs, config, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for edge_node in edge_cc:
            raw_intouched_nodes = set(mesh_nodes[edge_node].get('near')) if mesh_nodes[edge_node].get(
                'near') is not None else set()
            tmp_intouched_nodes |= set(
                [xx for xx in raw_intouched_nodes if mesh_nodes[xx].get('edge_id') is not None and \
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for i in range(max(background_thickness, context_thickness)):
                    ...
                    for ne in nes:
                        if bool(context_map[ne[0], ne[1]]) is False and \
                            ...
                            if (i - passive_background) % 2 == 0 and (i - passive_background) % 8 != 0:
                                four_nes = [xx for xx in [[ne[0] - 1, ne[1]], [ne[0] + 1, ne[1]], [ne[0], ne[1] - 1],
                                                          [ne[0], ne[1] + 1]] \
            ...
            new_tmp_mask_nodes = []
            for node in tmp_mask_nodes:
                four_nes = {xx: [] for xx in [(node[0] - 1, node[1]), (node[0] + 1, node[1]), (node[0], node[1] - 1),
                                              (node[0], node[1] + 1)] if \
                        ...
                        if i < background_thickness:
                            ...
                            if inpaint_iter == 0:
                                ...
                                if mesh_nodes[ne].get('edge_id') is not None and \
                                    ...
                                    intouched_fars = set(mesh_nodes[ne].get('far')) if mesh_nodes[ne].get(
                                        'far') is not None else set()
            ...
            for node in tmp_context_nodes:
                nes = mesh.neighbors(node)
                if inpaint_iter > 0:
                    four_nes = {(node[0] - 1, node[1]): [], (node[0] + 1, node[1]): [], (node[0], node[1] - 1): [],
                                (node[0], node[1] + 1): []}
                    for ne in nes: four_nes[(ne[0], ne[1])].append(ne[2])
                    nes = []
                    for kfne, vfnes in four_nes.items(): vfnes.sort(key=lambda xx: abs(xx), reverse=True)
                    ...
                    if bool(context_map[ne[0], ne[1]]) is False and mask_flag and \
                        ...
                        if i == passive_context and inpaint_iter == 1:
                            ...
                            if any([mask_map[mne[0], mne[1]] == True for mne in mnes]) is True:
                                ...
                                continue
                        if False and mesh_nodes[ne].get('near') is not None and mesh_nodes[ne].get(
                                'edge_id') != edge_id:
        ...
        if inpaint_iter == 0:
            depth_dict = get_depth_from_maps(
                context_map, mask_map, context_depth, mesh.graph['H'], mesh.graph['W'],
            ...
            depth_dict = depth_inpainting(
                None, None, None, None, mesh, config, union_size, depth_feat_model, None,
                given_depth_dict=depth_dict, spdb=False)
            near_depth_map, raw_near_depth_map = np.zeros((mesh.graph['H'], mesh.graph['W'])), np.zeros(
        ...
        try:
            if tmp_width == 0:
                ...
                erode_context_ccs[edge_id] = set(
                    functools.reduce(lambda x, y: x + y, [] + tmp_erode[:tmp_width])
                )
        except:
            raise
        ...
        extend_context_ccs[edge_id] = extend_context_ccs[edge_id] - mask_ccs[edge_id] - accomp_extend_context_ccs[
            edge_id]
    if inpaint_iter == 0:
        all_ecnt_cc = set()
        for ecnt_id, ecnt_cc in enumerate(extend_context_ccs):
            ...
            for constraint_context_id in constraint_context_ids:
                constraint_context_cc = constraint_context_cc | context_ccs[constraint_context_id] | erode_context_ccs[
            ...
            extend_context_ccs[ecnt_id] = extend_context_ccs[ecnt_id] - extend_erode_context_ccs[ecnt_id] - \
    ...
    ret = context_ccs, mask_ccs, broken_mask_ccs, edge_ccs, \
          erode_context_ccs, invalid_extend_edge_ccs, edge_maps, \
          extend_context_ccs, extend_edge_ccs, extend_erode_context_ccs
    return ret
...
def DL_inpaint_edge(
    ...
    if depth.ndim < 3:
        depth = depth[..., None]
    depth = np.expand_dims(depth.transpose(2, 0, 1), axis=0)
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(
        ...
        edge_dict = get_edge_from_nodes(
            ...
            erode_context_cc | extend_erode_context_ccs[edge_id], mask_cc, edge_cc,
        ...
        edge_dict['edge'], end_depth_maps, _ = \
            filter_irrelevant_edge_new(
                ...
                edge_id,
                edge_dict['context'],
                edge_dict['depth'], mesh,
                context_cc | erode_context_cc | \
                extend_context_cc | extend_erode_context_ccs[edge_id],
                spdb=False)
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            depth_edge_output = edge_forward_3P(
                ...
                tensor_edge_dict['rgb'], tensor_edge_dict['disp'],
                tensor_edge_dict['edge'])
        ...
        patch_edge_dict['output'] = tensor_edge_dict['output'].squeeze()
            ...
            if ((depth_edge_output > config['ext_edge_threshold']).astype(np.float) * tensor_edge_dict[
                ...
                try:
                    edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                        clean_far_edge_new(edge_dict['output'], end_depth_maps, edge_dict['mask'], edge_dict['context'],
                                           mesh, info_on_pix, edge_dict['self_edge'], inpaint_iter, config)
                except:
                    raise
                ...
                if config.get('repeat_inpaint_edge') is True:
                    for _ in range(2):
                        ...
                        full_depth_edge_output[union_size['x_min']:union_size['x_max'],
                        union_size['y_min']:union_size['y_max']] = \
                            depth_edge_output
                        edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                            clean_far_edge_new(full_depth_edge_output, end_depth_maps, edge_dict['mask'],
                                               edge_dict['context'], mesh, info_on_pix, edge_dict['self_edge'],
                                               inpaint_iter, config)
                for nid in npaths.keys():
                    ...
                    if len(valid_end_pt) > 0:
                        new_edge_info = dict(fpath=fpath[::-1],
                                             npath=npath[::-1],
                                             cont_end_pts=(
                                                 start_mx, start_my, info_on_pix[(start_mx, start_my)][0]['depth']),
                                             mask_id=edge_id,
                                             comp_edge_id=nid,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(
        ...
        if len(context_cc) < 1 or len(mask_cc) < 1:
            continue
        edge_dict = get_edge_from_nodes(context_cc | extend_context_cc,
                                        erode_context_cc | extend_erode_context_ccs[edge_id], mask_cc, edge_cc,
                                        extend_edge_ccs[edge_id],
                                        mesh.graph['H'], mesh.graph['W'], mesh)
        if specific_edge_loc is not None and \
        ...
        edge_dict['edge'], end_depth_maps, _ = \
            filter_irrelevant_edge_new(edge_dict['self_edge'], edge_dict['comp_edge'],
                                       ...
                                       edge_dict['context'],
                                       edge_dict['depth'], mesh,
                                       context_cc | erode_context_cc | extend_context_cc | extend_erode_context_ccs[
                                           edge_id], spdb=False)
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            edge_dict['fpath_map'], edge_dict['npath_map'] = edge_dict['fpath_map'] * 0 - 1, edge_dict[
                'npath_map'] * 0 - 1
            ...
            for end_pt in end_pts:
                ...
                if len(cur_info['cont_end_pts']) == 0 and len(other_infos) > 0:
                    ...
                    for other_info in other_infos:
                        tmp_fmap, tmp_nmap = np.zeros((mesh.graph['H'], mesh.graph['W'])) - 1, np.zeros(
                            (mesh.graph['H'], mesh.graph['W'])) - 1
                    ...
                    if min(tmp_fmap.max(), tmp_nmap.max()) != -1:
                        ...
                        discard_map = ((tmp_nmap != -1).astype(np.uint8) + (tmp_fmap != -1).astype(np.uint8)) * \
                                      edge_dict['mask']
                    else:
                        for fnode in cur_info['fpath']:
                            edge_dict['fpath_map'][fnode[0], fnode[1]] = cur_info['comp_edge_id']
                        for fnode in cur_info['npath']:
                            edge_dict['npath_map'][fnode[0], fnode[1]] = cur_info['comp_edge_id']
            edge_dict['output'] = (edge_dict['npath_map'] > -1) * edge_dict['mask'] + edge_dict['context'] * edge_dict[
                'edge']
        ...
        depth_dict = depth_inpainting(context_cc, extend_context_cc,
                                      erode_context_cc | extend_erode_context_ccs[edge_id], mask_cc, mesh, config,
                                      union_size, depth_feat_model, edge_dict['output'])
        ...
        rgb_dict = get_rgb_from_nodes(
            context_cc | extend_context_cc,
            erode_context_cc | extend_erode_context_ccs[edge_id], mask_cc, mesh.graph['H'],
            mesh.graph['W'], mesh)
        ...
        resize_rgb_dict = {k: v.copy() for k, v in tensor_rgb_dict.items()}
        ...
        if frac < 1:
            resize_mark = cv2.resize(
                np.concatenate(
                    (resize_rgb_dict['mask'], resize_rgb_dict['context']),
                    axis=1).transpose(0, 2, 3, 1)[0],
                None,
                fx=frac, fy=frac,
            ...
            resize_rgb_dict['edge'] = cv2.resize(
                resize_rgb_dict['edge'].transpose(0, 2, 3, 1)[0],
                None,
                fx=frac, fy=frac,
        ...
        rgb_output = color_forward_3P(
            rgb_model, specified_hole, resize_rgb_dict['context'],
        ...
        if frac < 1:
            tensor_rgb_dict['output'] = cv2.resize(
                tensor_rgb_dict['output'].transpose(0, 2, 3, 1)[0],
                tensor_rgb_dict['mask'].shape[-2:][::-1],
                interpolation=cv2.INTER_CUBIC).transpose(2, 0, 1)[None, :]
            tensor_rgb_dict['output'] = \
                tensor_rgb_dict['output'] * \
                tensor_rgb_dict['mask'] + (tensor_rgb_dict['rgb'] * tensor_rgb_dict['context'])
        patch_rgb_dict['output'] = tensor_rgb_dict['output'].squeeze().transpose(1, 2, 0)
        ...
        for hx, hy in zip(hxs, hys):
            ...
            if not (mesh.has_node(node)):
                ...
                mesh, _, _ = refresh_node((node[0], node[1]), mesh.nodes[(node[0], node[1])], node, ndict, mesh,
                                          stime=True)
...
def write_ply(image,
    ...
    input_mesh, info_on_pix = generate_init_node(input_mesh, min_node_in_cc=200)
    ...
    input_mesh, info_on_pix, edge_mesh, depth, aft_mark = \
        remove_dangling(
            input_mesh, edge_ccs, edge_mesh, info_on_pix,
            image, depth, config)

    input_mesh, depth, info_on_pix = update_status(
        input_mesh, info_on_pix, depth)
    edge_ccs, input_mesh, edge_mesh = group_edges(
        input_mesh, config, image, remove_conflict_ordinal=True)
    ...
    if config['extrapolate_border'] is True:
        ...
        input_mesh = remove_redundant_edge(
            input_mesh, edge_mesh, edge_ccs, info_on_pix, config,
            redundant_number=config['redundant_number'], spdb=False)
        input_mesh, depth, info_on_pix = update_status(input_mesh, info_on_pix, depth)
        ...
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            input_mesh, info_on_pix, image, depth,
            other_edge_with_id, edge_map, edge_ccs,
            depth_edge_model, depth_feat_model, rgb_model,
            config, direc="up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            input_mesh, info_on_pix, image, depth,
            other_edge_with_id, edge_map, edge_ccs,
        ...
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            ...
            other_edge_with_id, edge_map, edge_ccs,
            depth_edge_model, depth_feat_model, rgb_model,
            config, direc="right-up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            input_mesh, info_on_pix, image, depth,
            other_edge_with_id, edge_map, edge_ccs,
            depth_edge_model, depth_feat_model, rgb_model,
            config, direc="right-down")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            input_mesh, info_on_pix, image, depth,
            other_edge_with_id, edge_map, edge_ccs,
            depth_edge_model, depth_feat_model, rgb_model,
            config, direc="left-up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(
            input_mesh, info_on_pix, image, depth,
            other_edge_with_id, edge_map, edge_ccs,
            depth_edge_model, depth_feat_model, rgb_model,
            config, direc="left-down")
    specific_edge_loc = None
    specific_edge_id = []
    vis_edge_id = None
    ...
    init_mask_connect, edge_maps, extend_context_ccs, extend_edge_ccs, extend_erode_context_ccs = \
        context_and_holes(
            input_mesh, edge_ccs, config,
            specific_edge_id, specific_edge_loc, depth_feat_model,
            inpaint_iter=0, vis_edge_id=vis_edge_id
        )

    specific_edge_loc = None
    input_mesh, info_on_pix, specific_edge_nodes, new_edge_ccs, connect_points_ccs, image = \
        DL_inpaint_edge(
            input_mesh,
            info_on_pix,
            config,
    ...
    edge_maps, extend_context_ccs, extend_edge_ccs, extend_erode_context_ccs = \
        context_and_holes(
            input_mesh, new_edge_ccs, config,
            specific_edge_id, specific_edge_loc, depth_feat_model,
            connect_points_ccs, inpaint_iter=1
        )

    specific_edge_id = []
    input_mesh, info_on_pix, specific_edge_nodes, new_edge_ccs, _, image = \
        DL_inpaint_edge(
            input_mesh,
            info_on_pix,
            config,
